...
const ordertableService = {
  ...
  createOrderTable: async (name, tableState, numberOfChair , page =0 , size = 10) => {
    const token = getToken();
    if (!token) throw new Error('No authentication token found');
    ...
    const response = await fetch(`${API_URL}/table?page=${page}&size=${size}`, {
      ...
      method: 'POST',
      body: JSON.stringify({ name, tableState, numberOfChair }),
    });
    return response.json();
  },

  updateOrderTable: async (id, name, tableState, numberOfChair) => {
    const token = getToken();
    if (!token) throw new Error('No authentication token found');
