...
export const useLanguageService = () => {
  ...
  // Category services with current language
  const categoryServiceWithLang = {
    ...
    getCategoryById: (id) => 
      categoryService.getCategoryById(id, language),
    
    getCategoryView: (size = 100) => 
      categoryService.getCategoryView(size, language),
    
    // These don't need language parameter
  ...
  // Food services with current language
  const foodServiceWithLang = {
    ...
    getFoodById: (id) => 
      foodService.getFoodById(id, language),
    
    getFoodView: (size = 100) => 
      foodService.getFoodView(size, language),
